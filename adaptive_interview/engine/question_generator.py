...
class QuestionGeneratorEngine:
    def __init__(self, client: GeminiClient):
        self.client = client
        self.prompt_path = Path(__file__).parent.parent / "prompts" / "question_generator.txt"
    ...
    def generate(
        ...
        prompt = prompt.replace("{interview_blueprint}", json.dumps(interview_blueprint.model_dump(), indent=2))
        
        qb = self.client.generate_structured(prompt, QuestionBank)
        
        if len(qb.questions) != interview_blueprint.total_questions:
            raise ValueError(f"Question Generator returned {len(qb.questions)} questions, expected {interview_blueprint.total_questions}.")
